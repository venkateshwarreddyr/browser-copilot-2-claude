export const TOOL_DEFINITIONS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        tabId: { type: 'number', description: 'Tab ID to read from. Must be a tab in the current group. Use tabs_context first if you don\'t have a valid tab ID.' },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        tabId: { type: 'number', description: 'Tab ID to search in. Must be a tab in the current group. Use tabs_context first if you don\'t have a valid tab ID.' },
        max_results: { type: 'number', description: 'Maximum results to return (default: 20, max 50).' },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        tabId: { type: 'number', description: 'Tab ID to set form value in. Must be a tab in the current group. Use tabs_context first if you don\'t have a valid tab ID.' },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        duration: { type: 'number', minimum: 0, maximum: 30, description: 'The number of seconds to wait. Required for `wait`. Maximum 30 seconds.' },
        ...
        region: { type: 'array', items: { type: 'number' }, minItems: 4, maxItems: 4, description: '(x0, y0, x1, y1): The rectangular region to capture for `zoom`. Coordinates define a rectangle from top-left (x0, y0) to bottom-right (x1, y1) in pixels from the viewport origin. Required for `zoom` action.' },
        repeat: { type: 'number', minimum: 1, maximum: 100, description: 'Number of times to repeat the key sequence. Only applicable for `key` action. Default is 1.' },
        ...
        modifiers: { type: 'string', description: 'Modifier keys for click actions. Supports: "ctrl", "shift", "alt", "cmd" (or "meta"). Can be combined with "+" (e.g., "ctrl+shift", "cmd+alt"). Optional.' },
        tabId: { type: 'number', description: 'Tab ID to execute the action on. Must be a tab in the current group. Use tabs_context first if you don\'t have a valid tab ID.' },
      },
  ...
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        tabId: { type: 'number', description: 'Tab ID to extract text from. Must be a tab in the current group. Use tabs_context first if you don\'t have a valid tab ID.' },
        max_chars: { type: 'number', description: 'Maximum characters for output (default: 50000). Set to a higher value if your client can handle large outputs.' },
      },
      required: ['tabId'],
    },
  ...
  {
    name: 'extract_links',
    description: 'Extract links from the current page with text/title metadata.',
    input_schema: {
      type: 'object',
      properties: {
        tabId: { type: 'number', description: 'Tab ID to extract links from.' },
        max_results: { type: 'number', description: 'Maximum links to return (default: 100).' },
        same_domain_only: { type: 'boolean', description: 'Only include links from the same domain.' },
        include_hash: { type: 'boolean', description: 'Whether to keep URL fragment hashes.' },
      },
      required: ['tabId'],
    },
  },
  {
    ...
    description: 'Get text currently selected by the user on the page.',
    input_schema: {
      type: 'object',
      properties: {
        tabId: { type: 'number', description: 'Tab ID to read selection from.' },
  ...
  {
    name: 'page_snapshot',
    description: 'Return a compact structural snapshot of page stats, headings, and forms.',
    input_schema: {
      type: 'object',
      properties: {
        tabId: { type: 'number', description: 'Tab ID to snapshot.' },
        max_headings: { type: 'number', description: 'Maximum headings to return.' },
        max_forms: { type: 'number', description: 'Maximum forms to return.' },
      },
      required: ['tabId'],
    },
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        domains: { type: 'array', items: { type: 'string' }, description: 'List of domains you will visit (e.g., [\'github.com\', \'stackoverflow.com\']). These domains will be approved for the session when the user accepts the plan.' },
        approach: { type: 'array', items: { type: 'string' }, description: 'Ordered list of steps you will follow (e.g., [\'Navigate to homepage\', \'Search for documentation\', \'Extract key information\']). Be concise - aim for 3-7 steps.' },
      },
  ...
  {
    name: 'tabs_activate',
    description: 'Activate/focus a tab by tab ID.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        paths: { type: 'array', items: { type: 'string' }, description: 'The absolute paths to the files to upload. Can be a single file or multiple files.' },
        ref: { type: 'string', description: 'Element reference ID of the file input from read_page or find tools (e.g., "ref_1", "ref_2").' },
        tabId: { type: 'number', description: 'Tab ID where the file input is located. Use tabs_context first if you don\'t have a valid tab ID.' },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        coordinate: { type: 'array', items: { type: 'number' }, description: "Viewport coordinates [x, y] for drag & drop upload. Required for 'export' action unless 'download' is true." },
  ...
  },
  {
    ...
    description: 'Read browser console messages (console.log, console.error, console.warn, etc.) from a specific tab. Useful for debugging JavaScript errors, viewing application logs, or understanding what\'s happening in the browser console. Returns console messages from the current domain only. IMPORTANT: Always provide a pattern to filter messages - without a pattern, you may get too many irrelevant messages.',
    input_schema: {
      type: 'object',
      properties: {
        tabId: { type: 'number', description: 'Tab ID to read console messages from. Must be a tab in the current group. Use tabs_context first if you don\'t have a valid tab ID.' },
        ...
        pattern: { type: 'string', description: "Regex pattern to filter console messages. Only messages matching this pattern will be returned (e.g., 'error|warning' to find errors and warnings). You should always provide a pattern to avoid getting too many irrelevant messages." },
  ...
  },
  {
    ...
    description: 'Read HTTP network requests (XHR, Fetch, documents, images, etc.) from a specific tab. Useful for debugging API calls, monitoring network activity, or understanding what requests a page is making. Returns all network requests made by the current page, including cross-origin requests.',
    input_schema: {
      type: 'object',
      properties: {
        tabId: { type: 'number', description: 'Tab ID to read network requests from. Must be a tab in the current group. Use tabs_context first if you don\'t have a valid tab ID.' },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        tabId: { type: 'number', description: 'Tab ID to get the window for. Must be a tab in the current group. Use tabs_context first if you don\'t have a valid tab ID.' },
      },
  ...
  },
  {
    ...
    description: 'Execute JavaScript code in the context of the current page. The code runs in the page\'s context and can interact with the DOM, window object, and page variables. Returns the result of the last expression or any thrown errors. Do NOT use \'return\' statements - just write the expression you want to evaluate.',
    input_schema: {
      type: 'object',
      properties: {
        action: { type: 'string', description: 'Must be set to "javascript_exec"' },
        text: { type: 'string', description: 'The JavaScript code to execute. The code will be evaluated in the page context. The result of the last expression will be returned automatically. Do NOT use \'return\' statements - just write the expression you want to evaluate (e.g., \'window.myData.value\' not \'return window.myData.value\').' },
        tabId: { type: 'number', description: 'Tab ID to execute the code in. Must be a tab in the current group. Use tabs_context first if you don\'t have a valid tab ID.' },
      },
